...
void Chicken::updateSoundMade() {
    std::array<int, 2>currentTime = TimeManager::getTime();
    if ( m_numOfSoundPerDay != 0 && currentTime[1] == 6 ) {
        m_soundMade++;
        m_numOfSoundPerDay--;
    }
}
...
void Chicken::reproduce() {
    if ( m_age == 13 && m_weight == 2 && m_happyIndex == 10 ) {
        ...
        for ( int i = 1; i <= numberOfChild; i++ ) {
            ...
            newChicken->updateSoundMadeWithNumber(3);
            MacDonald::addAnimal( newChicken );
        }
    }
}

bool Chicken::ableToSell() {
    if ( m_weight == 2 ) {
        return true;
    }
    return false;
